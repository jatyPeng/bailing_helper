...
class XxlJobTaskHelper
{
    ...
    public function build($autoStart = false): void
    {
        ...
        //得到类的所有注解
        $classs = AnnotationCollector::getClassesByAnnotation(XxlJobTask::class);
    ...
    private function xxlJobGroupEdit($id, $appName, $title = '')
    {
        ...
        $this->getXxlJobData('jobgroup/update', [
            'appname' => $appName,
            'title' => $title,
            'addressType' => $this->groupList[$id]['addressType'],
            'addressList' => $this->groupList[$id]['addressList'],
            'id' => $id,
        ]);
     ...
     */
    private function getXxlJobData($uri, $formParams = [])
    {
        $client = new Client();
